...
import elasticsearch, unittest, logging, sys, os
from datetime import datetime
from elasticmock_additional_apis import elasticmock, behaviour
from .lightweight_curator import delete_indices

INDICES = ['app-000121', 'app-000122', 'app-000123', 'infra-000121', 'infra-000122', 'infra-000123', 'audit-000121', 'audit-000122', 'audit-000123']
...
class SetElasticMock(unittest.TestCase):
...
class TestCluster(SetElasticMock):
...
class TestIndices(SetElasticMock):
...
# Tests of Lightweight Curator
class TestDeleteIndices(SetElasticMock):

    def test_should_delete_list_of_indices_from_es(self):

        for index in INDICES:
            self.es.indices.create(index)

        delete_indices(self.es, INDICES)
        self.assertNotIn(True, [self.es.indices.exists(index) for index in INDICES])
